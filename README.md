# Météo Doline de Chaud Clapier — 24h

Page statique (GitHub Pages) affichant les prévisions ICON-CH1 (MétéoSuisse,
via Open-Meteo, gratuit et sans clé) pour la Doline de Chaud Clapier, sur la
...
4. **Configurer l'accès à ta station Datacake** (pour l'apprentissage
   ...
   - Récupère l'**UUID du device** (⚠️ différent de l'identifiant `d4b23fdc-...`
     de ton lien de dashboard public `/pd/...` — cet UUID se trouve dans les
     paramètres du device, ou via la requête GraphQL `allDevices`). ⚠️ Ce token
     et cet UUID sont ceux du **compte Datacake de la station de Chaud
     Clapier**, différent de celui utilisé pour le projet Gréolières.
